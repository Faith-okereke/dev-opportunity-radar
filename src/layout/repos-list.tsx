...
// Deterministic generator for 7 days of star growth history based on repo ID and stargazers count
const generateSparklinePoints = (repoId: number, stargazersCount: number) => {
  const points = [];
  let current = stargazersCount;
  for (let i = 0; i < 7; i++) {
    points.unshift(current);
    const seed = Math.sin(repoId + i) * 0.5 + 0.5; // between 0 and 1
    const dailyGrowthRate = 0.01 + seed * 0.04; // 1% to 5% growth
    current = Math.round(current / (1 + dailyGrowthRate));
  }
  return points;
};
...
const Sparkline = ({ points }: { points: number[] }) => {
  ...
  return (
    <svg width={width} height={height} className="overflow-visible" style={{ filter: "drop-shadow(0 0 1px rgba(59, 130, 246, 0.3))" }}>
      <path
        d={pathD}
        fill="none"
...
const LANGUAGES = [
  "All", "Assembly", "C", "C#", "C++", "Clojure", "Dart", "Elixir", "Go",
  "Haskell", "HTML", "Java", "JavaScript", "Julia", "Kotlin", "Lua", "Objective-C",
  "PHP", "Python", "R", "Ruby", "Rust", "Scala", "Shell", "Swift", "TypeScript", "Vue"
];
        ...
        {!isPreview && !profile.personalModeActive && (
          <div className="flex items-center gap-3">
            <input
              ...
              className="bg-white border border-slate-300 rounded-xl p-3 text-xs text-slate-900 focus:outline-none focus:border-[rgb(var(--color-accent))] transition-colors w-48"
            />
            <div className="relative">
              <button
                onClick={() => setIsLangDropdownOpen(!isLangDropdownOpen)}
                className="bg-white border border-slate-300 rounded-xl p-3 text-xs text-slate-900 focus:outline-none focus:border-[rgb(var(--color-accent))] transition-colors flex items-center justify-between min-w-[130px]"
              >
              ...
              {isLangDropdownOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-1 w-48 bg-white border border-slate-200 rounded-lg shadow-xl z-20 overflow-y-auto max-h-32 md:max-h-64 py-1">
                    {LANGUAGES.map(lang => (
                      <button
                        key={lang}
                        onClick={() => handleLanguageChange(lang)}
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-50 transition-colors ${language === lang ? "text-[rgb(var(--color-accent))] font-medium bg-slate-50" : "text-slate-700"}`}
                      >
          ...
          : displayRepos.map((repo: any) => {
            ...
            return (
              <a
                ...
                className={`flex flex-col min-w-full p-4 rounded-xl border transition-colors cursor-pointer ${isHot ? "bg-orange-500/10 border-orange-500/50 hover:border-orange-400" : "bg-slate-50 border-slate-200 hover:border-[rgb(var(--color-accent))]"
                  }`}
                key={repo.id}
              >
                <div className="flex justify-between items-start w-full mb-2">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div className="hidden sm:flex items-center mr-1" title="Star growth (last 7 days)">
                      <Sparkline points={generateSparklinePoints(repo.id, repo.stargazers_count)} />
                    </div>
