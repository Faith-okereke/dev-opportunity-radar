...
const JobsList = ({
  isPreview = false,
  ignorePersonalMode = false,
  onEmptyChange
}: {
  isPreview?: boolean;
  ignorePersonalMode?: boolean;
  ...
  if (profile.personalModeActive && !ignorePersonalMode) {
  ...
  useEffect(() => {
    if (onEmptyChange && !jobsLoading) {
      onEmptyChange(filteredJobs.length === 0 && rawJobs.length > 0);
    }
  }, [filteredJobs.length, rawJobs.length, jobsLoading, onEmptyChange]);
      ...
      <div className="flex items-center justify-between mb-4 pb-4 px-2 border-b border-slate-200 min-h-12.5">
        <div className="flex items-center gap-1">
          ...
          <p className="uppercase text-xs text-[rgb(var(--color-accent))] tracking-widest font-bold flex items-center gap-2">
            {profile.personalModeActive && !ignorePersonalMode ? "Your Perfect Matches" : "Job Market"}
            {!jobsLoading && <span className="bg-slate-100 text-slate-900 px-2 py-0.5 rounded-full text-[10px]">{filteredJobs.length}</span>}
          </p>
        </div>
        {!isPreview && (!profile.personalModeActive || ignorePersonalMode) && (
          <select
            value={keyword}
          ...
          : displayJobs.map((job: UnifiedJob) => {
            ...
            return (
              <a
                ...
                className={`flex flex-col min-w-full p-4 rounded-xl border border-slate-200 transition-all duration-200 hover:scale-[1.01] hover:shadow-sm cursor-pointer ${isHot ? "bg-orange-500/10" : "bg-slate-50"
                  }`}
                key={job.id}
              >
                <div className="flex justify-between items-start w-full mb-1 gap-2">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider ${job.source === 'HN' ? 'bg-[#ff6600]/20 text-[#ff6600]' : 'bg-blue-500/20 text-blue-400'
                      }`}>
                      {job.source}
                    </span>
                    {isHot && (
